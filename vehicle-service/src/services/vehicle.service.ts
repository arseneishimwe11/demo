import { PrismaClient, VehicleEntry, Ticket, TicketStatus } from '@prisma/client';
...
const getParkingLocation = async (parkingCode: string): Promise<ParkingLocation> => {
  try {
    const response = await axios.get(
      `${process.env.PARKING_SERVICE_URL || 'http://localhost:3003'}/api/parking/${parkingCode}`
    );
...
const updateParkingSpaces = async (parkingCode: string, change: number): Promise<void> => {
  try {
    await axios.put(
      `${process.env.PARKING_SERVICE_URL || 'http://localhost:3003'}/api/parking/${parkingCode}/spaces`,
      { change },
      {
        headers: {
...
export const registerVehicleEntry = async (data: VehicleEntryInput): Promise<{ entry: VehicleEntry; ticket: Ticket }> => {
...
export const registerVehicleExit = async (data: VehicleExitInput): Promise<VehicleEntry> => {
...
export const getVehicleEntryById = async (id: number): Promise<VehicleEntry | null> => {
...
export const generateBill = async (id: number): Promise<{ 
  entry: VehicleEntry; 
  ...
  if (entry.exitDatetime) {
    return {
      ...
      parkingDuration: Math.ceil(
        (entry.exitDatetime.getTime() - entry.entryDatetime.getTime()) / (1000 * 60 * 60)
      ),
