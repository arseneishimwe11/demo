...
export const entryVehicle = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (error instanceof Error && error.message === 'No available parking spaces') {
      return res.status(400).json({ error: error.message });
    }
    
    if (error instanceof Error && error.message.includes('Error fetching parking location')) {
...
export const exitVehicle = async (req: Request, res: Response) => {
  try {
    ...
    if (error instanceof Error && error.message === 'Vehicle has already exited') {
      return res.status(400).json({ error: error instanceof Error ? error.message : 'Vehicle has already exited' });
    }
